...
const std::string version = "Polarization CUDA 1.2.0";
...
void ExportAt(const std::string& path)
{
	...
	float* polarData = new float[wRender * hRender * 3];
	gpuErrchk(cudaMemcpy(polarData, gpuPolarData, wRender * hRender * 3 * sizeof(float), cudaMemcpyDeviceToHost));
	...
	for (int k = 0; k < 3; k++)
	{
		for (int i = hRender - 1; i >= 0; i--)
		{
			...
			{
				float value = polarData[(i * wRender + j) * 3 + k];
				if (isnan(value))
					value = 0.0f;
				fw << value << " ";
			}
...
void GuiRightBar()
{
	...
	if (ImGui::TreeNodeEx("Output Display", ImGuiTreeNodeFlags_SpanAvailWidth))
	{
		...
		const char* channels = "S0\0S1\0S2";
		int chnId = channel - 1;
		if (ImGui::Combo("##channel", &chnId, channels))
		{
			channel = chnId + 1;
			preview = false;
		}
				...
				for (int j = 0; j < objs[i].elements.size(); j++)
				{
						...
						elementName.c_str()))
					{
						...
						if (ImGui::Button(idSubStr.c_str(), ImVec2(65, 23)))
						{
							...
							if (imgPath.size() != 0)
							{
								previewer.SetIntensityTextureForElement(i, j, imgPath);
								sceneModified = true;
							}
						}
						...
						if (texId == -1)
							ImGui::BeginDisabled();
						idSubStr = "Remove##intensitymap" + idStr;
						if (ImGui::Button(idSubStr.c_str(), ImVec2(65, 23)))
						{
							previewer.SetIntensityTextureForElement(i, j, "");
							sceneModified = true;
						}
						if (texId == -1)
							ImGui::EndDisabled();
...
int InitializeGL(GLFWwindow*& window)
{
	...
	window = glfwCreateWindow(wWindow, hWindow, "PathTracer - Polarization CUDA", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return -1;
...
void InitializeFrame()
{
	...
	/*if (polarData)
		delete[] polarData;
	polarData = new float[wRender * hRender * 3];
	for (int i = 0; i < wRender * hRender * 3; i++)
		polarData[i] = 0.0f;*/

	if (gpuPolarData)
		gpuErrchk(cudaFree(gpuPolarData));
	gpuErrchk(cudaMalloc((void**)&gpuPolarData, wRender * hRender * 3 * sizeof(float)));
	gpuErrchk(cudaMemset(gpuPolarData, 0, wRender * hRender * 3 * sizeof(float)));
}
	...
	while (!glfwWindowShouldClose(window))
	{
		if (render)
		{
			...
			if (restart || init || stop)
			{
				...
				//pathTracer.SetOutImage(texData);
				//pathTracer.SetPolarData(polarData);
			...
			if (!canDraw)
			{
				...
				pathTracer.CUDARender(gpuTexData, gpuPolarData);
