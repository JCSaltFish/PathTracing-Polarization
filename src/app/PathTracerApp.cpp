...
void PathTracerApp::onFileDrop(const std::vector<std::string>& paths) {
    std::string scenePath{}; // path to the first .pls file
    for (const auto& path : paths) {
        std::string ext = std::filesystem::path(path).extension().string();
        if (ext == ".pls" && scenePath.empty())
            scenePath = path;
        else if (ext == ".obj")
            loadModelUtil(path);
    }
...
void PathTracerApp::updateUiMeshListItem(const DbObjHandle& hObj) {
    ...
    if (type == PtMesh::TYPE_NAME) {
        ...
        meshListItem->setWidgetValue(
            static_cast<int>(UiRightPanel::ID::MESH_INTENSITY),
            PtMaterial::getIntensity(hObj)
        );
        meshListItem->setWidgetValue(
        ...
        std::string intensityTexPath = PtMaterial::getIntensityTexPath(hObj);
        GfxImage intensityTex =
            AppTextureManager::instance().getIntensityPreviewTexture(intensityTexPath);
        meshListItem->intensityTexture =
            materialFlags.check(PtMaterial::MaterialFlag::INTENSITY_MAP) ?
...
void PathTracerApp::exportImage() const {
    ...
    if (m_pathTracer->getImageData(pixels, width, height))
    ...
    if (!fw.is_open())
        return;
    constexpr size_t pixelStride = 4;
    ...
    for (size_t k = 0; k < channelCount; ++k) {
        for (size_t i = height; i-- > 0;) {
            const size_t row_offset = i * rowLength;
            for (size_t j = 0; j < static_cast<size_t>(width); ++j) {
                const size_t pixel_offset = row_offset + (j * pixelStride) + k;
                float value = pixels[pixel_offset];
                if (std::isnan(value))
                    value = 0.0f;
                buffer << value;
                if (j < static_cast<size_t>(width) - 1)
                    buffer << ' ';
            }
            fw << buffer.str();
            buffer.str("");
            buffer.clear();
            if (!(k == channelCount - 1 && i == 0))
                fw << '\n';
        }
    }
    fw.flush();
...
void PathTracerApp::handleRightPanelEvent(const GuiEvent& event) {
    ...
    case UiRightPanel::ID::OUTPUT_DISP_CHANNEL:
    {
...
void PathTracerApp::handleMeshListEvent(const GuiEvent& event, const DbObjHandle& hModel) {
    ...
    case UiRightPanel::ID::MESH_INTENSITY:
    {
        float intensity = getEventValue<float>(*meshListCtx);
        if (meshListCtx->type == GuiEventType::COMMIT)
            DbUtils::txnFn(db, PtMaterial::setIntensity, hMaterial, intensity);
        break;
    }
...
(
    ...
    case UiRightPanel::ID::MESH_INTENSITY_TEX_LOAD:
    {
        ...
        materialFlags.set(PtMaterial::MaterialFlag::INTENSITY_MAP);
        DbUtils::TxnGuard txnGuard(AppDataManager::instance().getDB());
        if (PtMaterial::setFlags(hMaterial, materialFlags) != DB::Result::SUCCESS)
            break;
        if (PtMaterial::setIntensityTexPath(hMaterial, texturePath) != DB::Result::SUCCESS)
            break;
        txnGuard.commit();
        updateUiMeshListItem(hMaterial);
    ...
    case UiRightPanel::ID::MESH_INTENSITY_TEX_REMOVE:
    {
        removeMeshTexture(
            hMaterial,
            PtMaterial::MaterialFlag::INTENSITY_MAP,
            PtMaterial::setIntensityTexPath
        );
        break;
    }
    default:
