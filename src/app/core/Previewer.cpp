...
int Previewer::init(int resX, int resY, int samples) {
    ...
    m_descriptors.u_intensityTex.binding = 5;
    m_descriptors.u_intensityTex.type = GfxDescriptorType::SAMPLER;
    m_descriptors.u_intensityTex.stages.set(GfxShaderStage::FRAGMENT);
...
int Previewer::updateMesh(const DbObjHandle& hMesh, Mesh& mesh, const MeshDataInfo& meshDataInfo) {
    ...
    if (matFlags.check(MaterialFlag::INTENSITY_MAP) && mesh.material.textures.intensity) {
        bindings.push_back(
            { m_descriptors.u_intensityTex, mesh.material.textures.intensity }
        );
    } else
        bindings.push_back({ m_descriptors.u_intensityTex, defaultTexture });
...
int Previewer::updateMaterial(const DbObjHandle& hMaterial) {
    ...
    if (material->flags.check(MaterialFlag::INTENSITY_MAP)) {
        std::string texFile = PtMaterial::getIntensityTexPath(hMaterial);
        material->textures.intensity =
