...
int PostProcesser::init() {
    ...
    u_texture.binding = 0;
    u_texture.type = GfxDescriptorType::SAMPLER;
    u_texture.stages.set(GfxShaderStage::FRAGMENT);
...
int PostProcesser::initFrame(
    int width,
    int height,
    const std::array<GfxImage, 2>& inputImages
) {
    m_resolutionX = width;
...
void PostProcesser::setInputImage(GfxImage& image) {
...
int PostProcesser::renderFrame() {
    ...
    if (m_renderer->updateBufferData(m_uboParams, 0, sizeof(int), &m_dispChannel))
        return 1;
