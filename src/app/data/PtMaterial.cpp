...
void PtMaterial::serialize(DbSerializer& serializer, const PtMaterial& material) {
    ...
    serializer.serialize(material.m_intensity);
    ...
    serializer.serialize(material.m_intensityTexPath);
}
...
void PtMaterial::deserialize(DbSerializer& serializer, PtMaterial& material) {
    ...
    serializer.deserialize(material.m_intensity);
    ...
    serializer.deserialize(material.m_intensityTexPath);
}
...
float PtMaterial::getIntensity(const DbObjHandle& hMaterial) {
    const PtMaterial* material = view(hMaterial);
    if (!material)
        return 0.0f;
    return material->m_intensity;
}

DB::Result PtMaterial::setIntensity(const DbObjHandle& hMaterial, float intensity) {
    const PtMaterial* material = view(hMaterial);
    if (!material)
        return DB::Result::INVALID_HANDLE;
    if (material->m_intensity == intensity)
        return DB::Result::SUCCESS;
    PtMaterial newMaterial = *material;
    newMaterial.m_intensity = intensity;
    return hMaterial.getDB()->objModify(hMaterial, newMaterial);
}
...
std::string PtMaterial::getIntensityTexPath(const DbObjHandle& hMaterial) {
    const PtMaterial* material = view(hMaterial);
    if (!material)
        return std::string();
    return material->m_intensityTexPath.path;
}

DB::Result PtMaterial::setIntensityTexPath(const DbObjHandle& hMaterial, const std::string& path) {
    const PtMaterial* material = view(hMaterial);
    if (!material)
        return DB::Result::INVALID_HANDLE;
    if (material->m_intensityTexPath.path == path)
        return DB::Result::SUCCESS;
    PtMaterial newMaterial = *material;
    newMaterial.m_intensityTexPath.path = path;
    return hMaterial.getDB()->objModify(hMaterial, newMaterial);
}
