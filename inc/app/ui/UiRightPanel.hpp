...
public:
    ...
    enum class ID : int {
        ...
        MESH_INTENSITY_TEX_LOAD,
        MESH_INTENSITY_TEX_REMOVE,
    };
    ...
    UiRightPanel() {
        ...
        m_widgetStates[static_cast<int>(ID::OUTPUT_DISP_CHANNEL)].value = 0;
    ...
    void draw() override {
        ...
        text = GuiText::get("right_panel.output_disp.title");
        if (ImGui::TreeNodeEx(text.c_str(), ImGuiTreeNodeFlags_SpanAvailWidth)) {
            // Channel
            text = GuiText::get("right_panel.output_disp.channel");
    ...
    public:
        ...
        ImTextureID intensityTexture = 0;
        ...
        MeshListItem() {
            ...
            m_widgetStates[static_cast<int>(ID::MESH_INTENSITY)] = {};
            m_widgetStates[static_cast<int>(ID::MESH_INTENSITY)].value = 0.0f;
            ...
            m_widgetStates[static_cast<int>(ID::MESH_INTENSITY_TEX_LOAD)] = {};
            m_widgetStates[static_cast<int>(ID::MESH_INTENSITY_TEX_REMOVE)] = {};
        }
        ...
        void draw() override {
            ...
            if (expanded) {
                ...
                if (materialType == MaterialType::GLOSSY) {
                    ...
                    if (ImGui::SliderFloat(idStr.c_str(), &fValue, 0.0f, 1.0f, "%.2f", sliderFlags)) {
                ...
                // Intensity
                text = GuiText::get("right_panel.mesh_node.intensity");
                ImGui::Text("%s", text.c_str());
                ImGui::SameLine(160.0f * dpiScale);
                ImGui::SetNextItemWidth(150.0f * dpiScale);
                idStr = "##meshIntensity" + uidStr;
                fValue = getWidgetValue<float>(static_cast<int>(ID::MESH_INTENSITY));
                if (ImGui::InputFloat(idStr.c_str(), &fValue, 0.0f, 0.0f, "%.7f")) {
                    m_widgetStates[static_cast<int>(ID::MESH_INTENSITY)].value = fValue;
                    pushEvent(
                        {
                            LABEL,
                            static_cast<int>(ID::MESH_INTENSITY),
                            fValue,
                            GuiEventType::MODIFY
                        }
                    );
                ...
                posY = ImGui::GetCursorPosY();
                ImGui::SetCursorPosY(posY + (height - textHeight) * 0.5f);
                text = GuiText::get("right_panel.mesh_node.intensity_texture");
                ImGui::Text("%s", text.c_str());
                ImGui::SameLine(160.0f * dpiScale);
                ImGui::SetCursorPosY(posY);
                if (intensityTexture != 0)
                    ImGui::Image(intensityTexture, ImVec2(height, height));
                else
                    ImGui::Dummy(ImVec2(height, height));
                ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(0.0f, 0.5f));
                ImGui::PushFont(AppUiManager::instance().getNormalIconFont());
                ...
                ImGui::SetCursorPosY(posY);
                text = GuiText::get("right_panel.mesh_node.load");
                text += "##meshIntensityTexLoad" + uidStr;
                if (ImGui::Button(text.c_str(), ImVec2(65.0f * dpiScale, 0.0f))) {
                    pushEvent(
                        { LABEL, static_cast<int>(ID::MESH_INTENSITY_TEX_LOAD) }
                    );
                }
                ImGui::SetCursorPosX(posX);
                ImGui::SetCursorPosY(posY + height * 0.5f);
                if (intensityTexture == 0)
                    ImGui::BeginDisabled();
                text = GuiText::get("right_panel.mesh_node.remove");
                text += "##meshIntensityTexRemove" + uidStr;
                if (ImGui::Button(text.c_str(), ImVec2(65.0f * dpiScale, 0.0f))) {
                    pushEvent(
                        { LABEL, static_cast<int>(ID::MESH_INTENSITY_TEX_REMOVE) }
                    );
                }
                if (intensityTexture == 0)
                    ImGui::EndDisabled();
