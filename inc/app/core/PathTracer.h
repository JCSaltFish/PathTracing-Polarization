...
public:
     ...
     * @param[out] pixels Vector to store the pixel data (RGBA format).
     * @param[out] width Output parameter for the image width.
     * @param[out] height Output parameter for the image height.
     * @return 0 on success, non-zero on failure.
     */
    int getImageData(std::vector<unsigned char>& pixels, int& width, int& height) const;
...
private:
     ...
     */
    void loadModels(const DbObjHandle& hScene, BufferData& data);
    /**
...
private:
    ...
    GfxImage m_outImage = nullptr; // Output image

    GfxImage m_dspImageFront = nullptr; // Display image front buffer
    GfxImage m_dspImageBack = nullptr; // Display image back buffer
     ...
     */
    struct Descriptors {
        GfxDescriptor u_outImage = {}; // Output image descriptor
...
private:
    ...
    struct Material {
        ...
        uint32_t idxIntensityTex = 0; // Index of intensity texture
    };
    /**
