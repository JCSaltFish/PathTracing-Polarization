...
public:
     ...
     * @return 0 on success, non-zero on failure.
     */
    int initFrame(int width, int height, const std::array<GfxImage, 2>& inputImages);

    /**
     * @brief Set the input image for post-processing.
     * @param image Input image to be processed.
     */
    void setInputImage(GfxImage& image);
    /**
...
private:
    ...
    std::array<GfxImage, 2> m_inputImages = {}; // Input images for post-processing
    GfxImage m_currentInputImage = nullptr; // Current input image for post-processing
    ...
    GfxDescriptor u_texture = {}; // Descriptor for input texture
